...
OrcamentosController extends CrudController {
    ...
    public function acerta2Action() {  
        die;
        $inicio = new \DateTime('2016-05-01');
        $fim    = new \DateTime('2016-05-30 23:59:00');
        $adm    = 3234;
        $seguros = $this->getEm()
            ->createQueryBuilder()
            ->select('l,i,ld','lc')
...
//        echo var_dump($seguros->count()), '<br>';
    ...
    public function acerta3Action() { 
        ...
        $seguros = $this->getEm()
            ...
            ->select('l')
            ->from('Livraria\Entity\Orcamento', 'l')
            ->where('l.inicio BETWEEN :inicio AND :fim and l.administradora = :adm AND l.id BETWEEN :idIni AND :idFim')
            ->setParameter('inicio', $inicio)
            ->setParameter('fim', $fim)
            ->setParameter('adm', $adm)
            ->setParameter('idIni', 1137566)
            ->setParameter('idFim', 1137640)
            ->getQuery()
            ->getResult()
        ;
...
//        echo var_dump($seguros->count()), '<br>';
        ...
        $refImovels = array('35/25792/1','11/12655/1','19/11646/9','27/4211/1','27/29966/2','28/6186/1','11/1116/5','23/23624/1','11/1792/3','11/22689/1','19/3163/1','11/10178/1','17/9417/7','14/3527/61','19/8043/2','15/30020/1','11/14137/4','19/25116/6','11/1839/2','11/11281/3','19/8627/5','17/3656/21','27/15739/1','14/10569/12','27/27027/1','17/11194/5','27/4310/2','29/22948/1','28/6723/2','31/7659/5','28/25646/1','16/19931/1','17/3876/1','23/25694/1','23/11914/1','31/25392/1','27/4174/4','31/2840/10','15/3633/4','29/22434/1','11/2062/2','17/5592/10','15/21589/1','19/8387/5','14/10343/1','19/11646/3','31/2840/4','17/29917/1','19/8795/2','19/9473/9','19/9473/10','19/9473/11','19/3003/1','28/6304/2','11/7079/1','17/3955/3','11/1994/1','14/7383/2','28/21503/1','14/3705/4','11/12494/1','19/30005/1','27/11799/2','11/10614/2','28/23622/1','11/1545/3','19/29862/1','31/25697/1','27/20226/1','19/9569/1','17/10471/1','17/10401/3','17/10401/5','11/2418/12','14/3527/78','14/8465/1');
        
        foreach ($seguros as $seguro) {
            $key = array_search($seguro->getRefImovel(), $refImovels);
            if($key != null){
                unset($refImovels[$key]);
            }
        }
        echo '<pre>', var_dump($refImovels), '</pre>';
        
        echo ' total ', $ind, '<br>';
    }
    
    public function acertaAction() {
        $inicio = new \DateTime('2019-09-01');
        $fim    = new \DateTime('2019-09-31 23:59:00'); 
        echo '<pre>', var_dump($inicio), '</pre>';
        echo '<pre>', var_dump($fim), '</pre>';  
        $adm    = 3234;
        $seguros = $this->getEm()
            ->createQueryBuilder()
            ->select('l,i,ld','lc')
...
/**
 * Habitacion Mes 10/2015 somente os residenciais
 * 
 *    
$array[] = '1070771';
$array[] = '1070772';
...
$array[] = '1080772';
$array[] = '1080773';
//$array[] = '1080774'; duplica e excluido
$array[] = '1080775';
$array[] = '1080776';
$array[] = '1080777';
$array[] = '1080778';
        ...
        foreach ($array as $value) {
            ...
            if($orca){
                ...
                if($result === TRUE){
                    ...
                    echo '<h3> Erro ao recalcular ', $value, '</h3>';
                    foreach ($result as $key => $vlr) {
                        echo '<p> - mensagem ', $key, " - ", $vlr, '</p>';
                    }
                }
            }else{
            ...
            if($cont >= $force){
                echo 'importando mais 100 de 0 ate ', $cont, ' Aguarde !!' , date('d/m/Y - h:i'), ' <br>';
                @flush();
                $force += 100;
            }
        }      
        echo '<h1> Total ', $cont, '</h1>';
        echo ' Terminou !!' , date('d/m/Y - h:i'), ' <br>';
    }
